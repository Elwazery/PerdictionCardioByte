...
choice = st.sidebar.radio("", list(pages.keys()), index=0, format_func=lambda x: f"{'❤️' if x == 'Heart Disease Prediction' else ''} {x}")
...
with col3:
    # عرض مربع اختيار مستوى الكوليسترول للمستخدم
    cholesterol = st.selectbox(
        "Cholesterol level",
        options=["Normal", "Above normal", "High"]
    )

    # تحويل النص المُختار إلى قيمة رقمية مناسبة للنموذج
